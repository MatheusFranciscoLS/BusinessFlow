  ...
  if (!isClient && !selectedCompany) {
    return (
      <Container>
        <WelcomeBox>
          <div>
            <h1 style={{ margin: '0 0 8px 0', fontSize: 28 }}>Central de Comando</h1>
            <p style={{ margin: 0, opacity: 0.9, fontSize: 15 }}>Visão panorâmica e monitorização de todas as empresas do escritório.</p>
          </div>
          <Building2 size={60} opacity={0.2} />
        </WelcomeBox>

        {!agencySummary ? (
          <p style={{ color: '#a0aec0', textAlign: 'center', marginTop: 40 }}>A processar a inteligência do escritório...</p>
        ) : (
          <>
            <CardsGrid>
              <StatCard>
                <div className="title">MRR Global (Honorários) <DollarSign size={18} color="#38a169" /></div>
                <div className="value" style={{ color: '#38a169' }}>{formatCurrency(agencySummary.totalMRR)}</div>
              </StatCard>
              <StatCard>
                <div className="title">Aguardando Classificação <Clock size={18} color="#d69e2e" /></div>
                <div className="value" style={{ color: '#d69e2e' }}>{agencySummary.totalPending} documentos</div>
              </StatCard>
              <StatCard>
                <div className="title">Contas Atrasadas <AlertTriangle size={18} color="#e53e3e" /></div>
                <div className="value" style={{ color: '#e53e3e' }}>{agencySummary.totalOverdue} contas</div>
              </StatCard>
            </CardsGrid>
            ...
            <div style={{ background: 'white', borderRadius: 12, border: '1px solid #edf2f7', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                  ...
                  <tbody>
                    {agencySummary.companies.length === 0 ? (
                      <tr><td colSpan="5" style={{ padding: 24, textAlign: 'center', color: '#a0aec0' }}>Nenhuma empresa cadastrada.</td></tr>
                    ) : (
