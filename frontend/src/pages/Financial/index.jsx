...
import { 
  ...
  UploadCloud, Link as LinkIcon, CreditCard
} from 'lucide-react';
...
export default function Financial() {
    ...
    // 🔥 Função de Exportar PDF Restaurada!
    function handleExportPDF() {
        ...
        const clientName = selectedCompany?.name || 'Visão Geral (Escritório)';
        ...
        const tableRows = filteredTransactions.map(t => {
            ...
            const clientInfo = t.client ? t.client.fullName : 'Geral';
            return [
    ...
    function handleEdit(t) {
        ...
        setClientId(t.clientId || ''); setFile(null); setIsRecurring(false); setInstallments(1); 
        setIsModalOpen(true);
    }
    ...
    async function handleMarkAsPaid(t) {
        ...
        try {
            ...
            formData.append('title', t.title || t.description); formData.append('description', t.description || t.title);
            formData.append('amount', t.amount || t.price); formData.append('category', t.category);
            formData.append('type', t.type); formData.append('date', new Date(t.date).toISOString());
    ...
    async function handleSave(e) {
        ...
        try {
            ...
            const formData = new FormData();
            formData.append('title', title); formData.append('description', title); formData.append('amount', price);
            formData.append('category', category); formData.append('type', apiType); formData.append('date', new Date(date).toISOString());
    ...
    async function handleOfxUpload(e) {
        ...
        try {
            ...
            toast.success(`${data.length} transações encontradas!`, { id: tId });
        } catch (error) {
            toast.error("Erro ao ler o ficheiro OFX.", { id: tId });
        }
    }
    ...
    async function handleConciliate() {
        ...
        const tId = toast.loading("A conciliar...");
        try {
            const formData = new FormData();
            formData.append('status', 'PAGO');
            ...
            mutate(); 
            toast.success("Match perfeito! Conta conciliada.", { id: tId });
        } catch (error) { toast.error("Erro ao conciliar.", { id: tId }); }
    }
        ...
        <Container>
            <Header style={{ marginBottom: 32 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, marginBottom: 24 }}>
                    ...
                    <ButtonGroup>
                        <button className="secondary" onClick={() => setIsOfxModalOpen(true)} disabled={isClient} style={{ display: isClient ? 'none' : 'flex', background: '#ebf8ff', color: '#3182ce', borderColor: '#bee3f8' }}><LinkIcon size={18} /> Conciliar OFX</button>
                ...
                <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap', width: '100%', marginBottom: 16 }}>
                    <SearchContainer style={{ flex: '1 1 250px', maxWidth: 'none', margin: 0 }}><Search size={20} color="#a0aec0" /><input placeholder="Buscar por descrição ou cliente..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} disabled={!transactions} /></SearchContainer>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <MonthNavigator style={{ opacity: showAllTime ? 0.4 : 1, pointerEvents: showAllTime ? 'none' : 'auto', margin: 0 }}><button onClick={handlePreviousMonth}><ChevronLeft size={20} /></button><span>{MONTHS_BR[currentMonth - 1]} de {currentYear}</span><button onClick={handleNextMonth}><ChevronRight size={20} /></button></MonthNavigator>
                        <button onClick={() => setShowAllTime(!showAllTime)} style={{ height: 48, padding: '0 16px', borderRadius: 8, border: '1px solid', background: showAllTime ? '#ebf8ff' : 'white', color: showAllTime ? '#3182ce' : '#718096', borderColor: showAllTime ? '#3182ce' : '#e2e8f0', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>{showAllTime ? 'Filtrar por Mês' : 'Todo o Histórico'}</button>
                    </div>
                </div>

                <FilterPillsContainer>{categories.map(cat => (<FilterPill key={cat} $active={filterCategory === cat} onClick={() => setFilterCategory(cat)}>{cat}</FilterPill>))}</FilterPillsContainer>
            </Header>
               ...
               <><SummaryContainer><SkeletonRow style={{ height: 120 }} /><SkeletonRow style={{ height: 120 }} /><SkeletonRow style={{ height: 120 }} /></SummaryContainer><TableContainer style={{ padding: 24 }}><SkeletonRow /><SkeletonRow /><SkeletonRow /><SkeletonRow /></TableContainer></>
                ...
                <>
                    ...
                    {filteredTransactions.length === 0 ? (<p style={{textAlign: 'center', marginTop: 40, color: '#a0aec0'}}>Nenhuma movimentação encontrada neste período.</p>) : (
                        <TableContainer>
                            <Table>
                                <thead><tr><th>Descrição / Conta</th><th>Cliente Vinculado</th><th>Valor</th><th>Categoria</th><th>Data</th><th>Situação</th><th style={{ textAlign: 'center' }}>Anexo</th><th style={{ textAlign: 'right' }}>Ações</th></tr></thead>
                                <tbody>
                                    {filteredTransactions.map(t => {
                                        ...
                                        return (
                                            <tr key={t.id} style={{ opacity: isNotPaid ? 0.8 : 1, background: t.status === 'ATRASADO' ? '#fff5f5' : 'transparent' }}>
                                                ...
                                                <td>{t.client ? (<span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#4a5568', fontWeight: 600 }}><User size={14} color="#a0aec0" /> {t.client.fullName}</span>) : (<span style={{ color: '#cbd5e0', fontSize: 13, fontStyle: 'italic' }}>Geral</span>)}</td>
                                                ...
                                                <td style={{ textAlign: 'center' }}>{t.fileUrl ? (<button onClick={() => openAttachment(t.fileUrl)} title="Ver Documento" style={{ background: '#ebf8ff', border: 'none', padding: '6px', borderRadius: '6px', cursor: 'pointer', color: '#3182ce', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}><Paperclip size={18} /></button>) : (<span style={{ color: '#cbd5e0', fontSize: 12 }}>-</span>)}</td>
                                                <td style={{ textAlign: 'right' }}>
                                                    ...
                                                    {!isClient && (<><ActionButton onClick={() => handleEdit(t)} color="#3182ce" style={{ marginRight: 8 }}><Edit size={18} /></ActionButton><ActionButton onClick={() => handleDelete(t.id)} color="#e53e3e"><Trash2 size={18} /></ActionButton></>)}
                                                </td>
                                            </tr>
                                        );
                                    })}
                ...
                <ModalOverlay>
                    <ModalContent style={{ maxWidth: 1000, background: '#f8fafc', padding: '32px 40px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                            <div><h2 style={{ margin: '0 0 4px 0', color: '#1a202c', display: 'flex', alignItems: 'center', gap: 8 }}><LinkIcon color="#3182ce" /> Conciliação Bancária</h2><p style={{ margin: 0, color: '#718096', fontSize: 14 }}>Importe o extrato do banco (.OFX) e faça o cruzamento com as contas pendentes.</p></div>
                        ...
                        {bankTransactions.length === 0 ? (
                            <div style={{ border: '2px dashed #cbd5e0', padding: '60px 20px', borderRadius: '16px', textAlign: 'center', background: 'white', position: 'relative' }}><input type="file" onChange={handleOfxUpload} accept=".ofx" style={{ opacity: 0, position: 'absolute', top:0, left:0, width:'100%', height:'100%', cursor:'pointer' }} /><UploadCloud size={48} color="#3182ce" style={{ marginBottom: 16 }} /><h3 style={{ margin: '0 0 8px 0', color: '#2d3748' }}>Arraste o ficheiro .OFX do banco para aqui</h3><p style={{ margin: 0, color: '#a0aec0' }}>Ou clique para procurar no computador.</p></div>
                        ) : (
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, height: '60vh' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                                    <div style={{ padding: 16, background: '#edf2f7', borderBottom: '1px solid #e2e8f0', fontWeight: 700, color: '#2d3748' }}>🏦 Extrato do Banco ({bankTransactions.length})</div>
                                    <div style={{ flex: 1, overflowY: 'auto', padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {bankTransactions.map(bt => (
                                            <div key={bt.id} onClick={() => setSelectedBankTx(bt)} style={{ padding: 16, borderRadius: 8, border: `2px solid ${selectedBankTx?.id === bt.id ? '#3182ce' : '#e2e8f0'}`, background: selectedBankTx?.id === bt.id ? '#ebf8ff' : 'white', cursor: 'pointer', transition: 'all 0.2s' }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}><span style={{ fontSize: 14, fontWeight: 700, color: '#2d3748' }}>{bt.description}</span><span style={{ fontWeight: 800, color: bt.type === 'entrada' ? '#12a454' : '#e53e3e' }}>{bt.type === 'saida' && '- '}{formatCurrency(bt.amount)}</span></div><span style={{ fontSize: 12, color: '#a0aec0' }}>Data: {formatDateDisplay(bt.date)}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                                    <div style={{ padding: 16, background: '#fefcbf', borderBottom: '1px solid #e2e8f0', fontWeight: 700, color: '#975a16' }}>⏳ Pendentes no BusinessFlow</div>
                                    <div style={{ flex: 1, overflowY: 'auto', padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {pendingTransactions.length === 0 ? (<div style={{ textAlign: 'center', padding: 40, color: '#a0aec0' }}>Nenhuma conta pendente para conciliar!</div>) : (
                                            pendingTransactions.map(pt => (
                                                <div key={pt.id} onClick={() => setSelectedSystemTx(pt)} style={{ padding: 16, borderRadius: 8, border: `2px solid ${selectedSystemTx?.id === pt.id ? '#d69e2e' : '#e2e8f0'}`, background: selectedSystemTx?.id === pt.id ? '#fffff0' : 'white', cursor: 'pointer', transition: 'all 0.2s' }}>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}><span style={{ fontSize: 14, fontWeight: 700, color: '#2d3748' }}>{pt.title}</span><span style={{ fontWeight: 800, color: (pt.type === 'entrada' || pt.type === 'income') ? '#12a454' : '#e53e3e' }}>{formatCurrency(pt.amount || pt.price)}</span></div>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 12, color: '#a0aec0' }}>Vence: {formatDateDisplay(pt.date)}</span>{pt.client && <span style={{ fontSize: 11, background: '#edf2f7', padding: '2px 6px', borderRadius: 4, color: '#4a5568' }}>{pt.client.fullName}</span>}</div>
                                                </div>
                                            ))
                                        )}
                                    </div>
                                ...
                                <div style={{ gridColumn: '1 / -1', background: 'white', padding: 20, borderRadius: 12, border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                        <div style={{ textAlign: 'center' }}><div style={{ fontSize: 12, color: '#718096', fontWeight: 600, textTransform: 'uppercase' }}>Selecionado (Banco)</div><div style={{ fontSize: 18, fontWeight: 800, color: selectedBankTx ? (selectedBankTx.type === 'entrada' ? '#12a454' : '#e53e3e') : '#a0aec0' }}>{selectedBankTx ? formatCurrency(selectedBankTx.amount) : 'R$ 0,00'}</div></div>
                                        <LinkIcon size={24} color="#cbd5e0" />
                                        <div style={{ textAlign: 'center' }}><div style={{ fontSize: 12, color: '#718096', fontWeight: 600, textTransform: 'uppercase' }}>Selecionado (Sistema)</div><div style={{ fontSize: 18, fontWeight: 800, color: selectedSystemTx ? ((selectedSystemTx.type === 'entrada' || selectedSystemTx.type === 'income') ? '#12a454' : '#e53e3e') : '#a0aec0' }}>{selectedSystemTx ? formatCurrency(selectedSystemTx.amount || selectedSystemTx.price) : 'R$ 0,00'}</div></div>
                                    </div>
                                    <button onClick={handleConciliate} disabled={!selectedBankTx || !selectedSystemTx} style={{ background: (!selectedBankTx || !selectedSystemTx) ? '#cbd5e0' : '#3182ce', color: 'white', border: 'none', padding: '16px 32px', borderRadius: 8, fontSize: 16, fontWeight: 700, cursor: (!selectedBankTx || !selectedSystemTx) ? 'not-allowed' : 'pointer', transition: '0.2s', boxShadow: '0 4px 6px rgba(49, 130, 206, 0.2)' }}>Conciliar Transações</button>
                                </div>
                            </div>
                        )}
                    </ModalContent>
                ...
                <ModalOverlay>
                    <ModalContent style={{ maxWidth: 650 }}>
                        ...
                        <form onSubmit={handleSave}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}><FormGroup><label>Descrição do Lançamento</label><input value={title} onChange={e => setTitle(e.target.value)} required placeholder="Ex: Honorários Mensais" /></FormGroup><FormGroup><label>Valor (R$)</label><input type="number" step="0.01" value={price} onChange={e => setPrice(e.target.value)} required /></FormGroup></div>
                            <FormGroup><label>Vincular a um Cliente do CRM (Opcional)</label><select value={clientId} onChange={e => setClientId(e.target.value)} style={{ padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#f7fafc' }}><option value="">Não vincular (Despesa Interna/Geral)</option>{clients?.map(c => (<option key={c.id} value={c.id}>{c.fullName} - (CNPJ: {c.document || 'N/A'})</option>))}</select></FormGroup>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}><FormGroup><label>Categoria</label><select value={category} onChange={e => setCategory(e.target.value)} required style={{ padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}><option value="">Selecione a conta...</option>{(type === 'income' || type === 'entrada') ? (<optgroup label="RECEITAS"><option value="Honorários Contábeis">Honorários Mensais</option><option value="Serviços Extras">Serviços Extras</option></optgroup>) : (<optgroup label="CUSTOS"><option value="Folha de Pagamento">Folha de Pagamento</option><option value="Impostos">Impostos (Simples, DARF)</option><option value="Despesas Fixas">Despesas Fixas (Software, Aluguel)</option></optgroup>)}</select></FormGroup><FormGroup><label>Data de Vencimento</label><input type="date" value={date} onChange={e => setDate(e.target.value)} required /></FormGroup></div>
                            {!editingId && (<div style={{ background: '#f7fafc', border: '1px solid #edf2f7', padding: '16px', borderRadius: '8px', marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}><label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 600, color: '#2d3748' }}><input type="checkbox" checked={isRecurring} onChange={(e) => setIsRecurring(e.target.checked)} style={{ width: '18px', height: '18px' }} /><Repeat size={18} color="#3182ce" /> Lançamento Recorrente (Mensalidade)</label>{isRecurring && (<div style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingLeft: '26px' }}><span style={{ fontSize: '13px', color: '#4a5568' }}>Repetir por</span><input type="number" min="2" max="60" value={installments} onChange={(e) => setInstallments(e.target.value)} style={{ width: '80px', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e0' }} /><span style={{ fontSize: '13px', color: '#4a5568' }}>meses</span></div>)}</div>)}
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}><FormGroup><label>Situação</label><select value={status} onChange={e => setStatus(e.target.value)} style={{ padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}><option value="PAGO">✅ Pago (Em Caixa)</option><option value="PENDENTE">⏳ Pendente</option></select></FormGroup><FormGroup><label>Meio de Pagamento</label><select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} style={{ padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}><option value="">Não informado</option><option value="PIX">PIX</option><option value="Boleto Bancário">Boleto</option></select></FormGroup></div>
                            <ModalActions><button type="button" className="cancel" onClick={() => setIsModalOpen(false)}>Cancelar</button><button type="submit" className="save">Confirmar Lançamento</button></ModalActions>
                        </form>
                    </ModalContent>
                </ModalOverlay>
            )}
